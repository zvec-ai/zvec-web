...
fused_results = reranker.rerank(query_results, topn=3)
```
...
# Prepare results and their corresponding vector schemas
query_results = [
    [zvec.Doc(id="1", score=0.8), zvec.Doc(id="2", score=0.7)],
    [zvec.Doc(id="2", score=0.9), zvec.Doc(id="3", score=0.75)],
]
fields = [
    zvec.VectorSchema(
...
reranker = zvec.WeightedReRanker(
    weights=[0.7, 0.3],  # One weight for each result list, in the same order
)
fused_results = reranker.rerank(
    query_results,
    topn=3,
    fields=fields,  # Required for score normalization in standalone rerank()
)
print(fused_results)
```

When `WeightedReRanker` is passed to `Collection.query()`, Zvec obtains the field schemas automatically:

```python
fused_results = collection.query(
    queries=[
        zvec.Query(field_name="dense_embedding", vector=[0.1] * 768),
        zvec.Query(field_name="sparse_embedding", vector={1: 0.1, 37: 0.43}),
    ],
    topk=3,
    reranker=zvec.WeightedReRanker(weights=[0.7, 0.3]),
)
```
...
# Recall top-100, then rerank the candidate list
candidates = collection.query(
    queries=Query(field_name="dense", vector=query_vec),
    topk=100,
)
final_results = reranker.rerank([candidates], topn=10)
```
...
rrf_ranker = RrfReRanker(rank_constant=60)

# Query both vector fields and fuse the results
final_results = collection.query(
    queries=[
        Query(field_name="dense", vector=dense_vec),
        Query(field_name="sparse", vector=sparse_vec),
    ],
    topk=3,
    reranker=rrf_ranker,
)
