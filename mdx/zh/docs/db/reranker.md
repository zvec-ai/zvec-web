...
reranker = zvec.WeightedReRanker(
    weights=[0.7, 0.3],  # 按相同顺序为每个结果列表设置一个权重
)
fused_results = reranker.rerank(
    query_results,
    topn=3,
    fields=fields,  # 直接调用 rerank() 时，评分归一化需要此参数
)
print(fused_results)
```

将 `WeightedReRanker` 传给 `Collection.query()` 时，Zvec 会自动获取字段 Schema：
