...
Vue 与 Svelte 的脚本块也会在可能时使用结构化提取。
...
GIF、JPEG、PNG 与 WebP 默认不进入索引。必须显式包含，并选择能够处理图片的 Embedding 模型。

## 当前跳过的内容

<Callout type="warn" className="text-base">
  PDF 与 Office 文件、压缩包、编译产物、音视频、数据库文件、空文件、检测到的二进制文件及超过大小限制的文件不会进入索引。
</Callout>
