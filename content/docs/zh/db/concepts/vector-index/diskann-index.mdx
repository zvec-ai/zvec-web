...
<Callout className="text-base" type="warn">
  **平台要求**：DiskANN 目前**仅支持 Linux** 平台，并且需要系统安装 **[libaio](https://pagure.io/libaio)**（Linux 异步 I/O）库。

  预编译包基于 **`libaio1`** 编译。部分较新的 Linux 发行版（如 Ubuntu 24.04+）默认提供的是 **`libaio1t64`**，由于 ABI 不兼容，可能导致链接时找不到库的错误。如果遇到链接问题，可以在您的环境中从源码编译 Zvec，以链接系统上可用的 libaio 版本。
</Callout>
