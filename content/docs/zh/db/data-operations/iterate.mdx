...
```python tab="Python" title="遍历所有 Document" tab-group="code-demo"
# [!code word:iter_docs]
...
```ts tab="Node.js" title="遍历所有 Document"
...
```python tab="Python" title="选择输出字段并排除向量" tab-group="code-demo"
...
```ts tab="Node.js" title="选择输出字段并排除向量"
...
```python tab="Python" title="提前结束遍历" tab-group="code-demo"
with collection.iter_docs() as docs:
    for doc in docs:
