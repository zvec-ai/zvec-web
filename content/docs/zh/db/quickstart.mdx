---
title: 快速开始
description: 开箱即用，几分钟内轻松上手 Zvec：创建 collection、写入 document、执行向量相似度检索。
icon: Rocket
---
...
<Callout type="info">
  Python 包需要 **64 位** Python 解释器（3.9–3.14），不支持 32 位解释器。
</Callout>
...
```python tab="Python" title="创建 collection" tab-group="code-demo"
import zvec
...
```python tab="Python" title="插入 document" tab-group="code-demo"
# [!code word:embedding]
# [!code word:publish_year]
...
```python tab="Python" title="相似度检索" tab-group="code-demo"
# [!code word:embedding]
...
```python tab="Python" title="带条件过滤的相似度检索" tab-group="code-demo"
# [!code word:embedding]
result = collection.query(        # [!code highlight]
    queries=zvec.Query(
        field_name="embedding",
...
```ts tab="Node.js" title="带条件过滤的相似度检索"
// [!code word:embedding]
// 同步
let result = collection.querySync({     // [!code highlight]
    fieldName: "embedding",
...
```python tab="Python" title="查看 collection schema" tab-group="code-demo"
# [!code word:schema]
...
```python tab="Python" title="查看 collection 的统计信息" tab-group="code-demo"
# [!code word:stats]
...
```python tab="Python" title="删除 document" tab-group="code-demo"
# [!code word:delete]
...
```python tab="Python" title="按筛选条件删除 document" tab-group="code-demo"
