...
apiReference: /api-reference/db/#zvec.db.create_and_open
---


import { Accordion, Accordions } from 'fumadocs-ui/components/accordion';

To create a new, empty collection in Zvec, you must define two key components:

- **Schema** —  the structural blueprint of your data, specifying scalar fields and vector representations.
- **Collection options** — runtime settings that control how the collection behaves when opened (e.g., read-only mode).
...
<Accordions type="single">
  ...
  from centaurdb.model.param import InvertIndexParam
  from centaurdb.model.schema import FieldSchema
  from centaurdb.typing import DataType

  field_schema = FieldSchema(   # [!code highlight]
...
1. 📐 `dimension`: Required for [dense vectors](../../concepts/vector-embedding/#dense-vectors) — the number of dimensions.
...
<Accordions type="single">
  ...
  vector_schema = VectorSchema(   # [!code highlight]
    name="dense_vector",
...
<Accordions type="single">
  ...
  from centaurdb.model.param import HnswIndexParam, InvertIndexParam
...
from centaurdb.model.param import CollectionOption

collection_option = CollectionOption(read_only=False, enable_mmap=True)   # [!code highlight]
```
...
<Accordions type="single">
  ...
  from centaurdb.model.schema import FieldSchema, VectorSchema
  from centaurdb.typing import DataType, MetricType, QuantizeType
  ...
  collection_schema = db.CollectionSchema(  # [!code highlight]
      ...
      fields=[                              # [!code highlight]
          FieldSchema(
              name="image_url",
              data_type=DataType.STRING,    # Not used in filtering, no index created
      ...
      vectors=[                             # [!code highlight]
          ...
          VectorSchema(
              ...
              index_param=HnswIndexParam(
                  metric_type=MetricType.COSINE, quantize_type=QuantizeType.INT8
              ),
          ),
