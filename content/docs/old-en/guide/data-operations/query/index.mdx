...
```python
from centaurdb.model import VectorQuery
from centaurdb.extension import WeightedReRanker
from centaurdb.typing import MetricType

result = collection.query(  # [!code highlight]
    topk=10,
    vectors=[
        VectorQuery(field_name="dense_embedding", vector=[0.1] * 768,           # [!code highlight]
        VectorQuery(field_name="sparse_embedding", vector={1: 0.1, 37: 0.43}),  # [!code highlight]
    ],
    reranker=WeightedReRanker(                                                  # [!code highlight]
        topn=3,
