...
<Accordions type="single">
  ...
  vector_query = VectorQuery(  # [!code highlight]
      field_name="dense_embedding",
      vector=[0.1] * 768,  # Replace with a real 768-dim embedding in practice
  )

  by_id_query = VectorQuery(  # [!code highlight]
      field_name="dense_embedding",
...
<Accordions type="single">
  ...
  result = collection.query(    # [!code highlight]
      vectors=VectorQuery(
          ...
          vector={42: 1.25, 1337: 0.8, 1999: 0.64},   # Replace with a real sparse embedding in practice
      ),
      topk=3,
  )
  print(result)
...
<Accordions type="single">
  ...
  from centaurdb.model import VectorQuery
  from centaurdb.model.param import HnswQueryParam

  result = collection.query(    # [!code highlight]
      vectors=VectorQuery(
          field_name="dense_embedding",
          vector=[0.1] * 768,
          param=HnswQueryParam(ef=500),  # Set ef to a larger value for better recall [!code highlight]
      ),
      topk=10,
  )
  print(result)
