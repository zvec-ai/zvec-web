...
- **RaBitQ for distance estimation** 🔍
  ...
  1. During **search**, the query vector is transformed using the same rotation, and distances to candidate nodes are *estimated* using SIMD instructions on the quantized data — instead of full FP32 distance computations.
