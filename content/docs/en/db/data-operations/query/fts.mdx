...
<Callout className="text-base" type="warn">
  Leading `NOT` is not supported — `NOT` is a binary operator only. Use `-term` to exclude a term, or write `a NOT b` to exclude `b` from results matching `a`.
</Callout>
...
<Callout className="text-base" type="warn">
  FTS queries and vector queries are **mutually exclusive** in a single `Query` object. You cannot combine `fts` with `vector` or `id` in the same query.
</Callout>
...
| --------- | ---- | ----------- |
| Standard | `"standard"` | Splits text on non-alphanumeric characters. Best for most Latin-script languages. **(Default)** |
...
<Callout className="text-base" type="warn">
  ...
  - FTS queries and vector queries are **mutually exclusive** in a single `Query` — you cannot combine `fts` with `vector` or `id`.
  ...
  - Leading `NOT` is a parse error — use `a NOT b` or `-term` instead.
