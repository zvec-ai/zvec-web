...
```python tab="Python" title="Define a field with full-text index" tab-group="code-demo"
import zvec
...
```ts tab="Node.js" title="Define a field with full-text index"
import { ZVecDataType, ZVecFieldSchema, ZVecIndexType } from "@zvec/zvec";
...
vector_schema = zvec.VectorSchema(  # [!code highlight]
    ...
    index_param=zvec.HnswIndexParam(
        metric_type=zvec.MetricType.COSINE, ef_construction=700, quantize_type=QuantizeType.INT8, quantizer_param=QuantizerParam(enable_rotate=True)
    ),
)
```
