...
1. `index_param` (optional): Enables fast filtering via `InvertIndexParam` ([inverted index](../../../concepts/inverted-index/)) or full-text search via `FtsIndexParam` ([full-text index](../../../concepts/fts-index/)).
...
<Accordion title="Inverted Index Example">

```python tab="Python" title="Define a scalar field with inverted index" tab-group="code-demo"
import zvec
...
```ts tab="Node.js" title="Define a scalar field with inverted index"
...
<Accordion title="Full-Text Index Example">

Use `FtsIndexParam` to enable [full-text search](../../../data-operations/query/fts/) on a `STRING` field. The field content will be tokenized and indexed for keyword retrieval with BM25 relevance ranking.
