...
```python tab="Python" title="Add a column"
import zvec
...
- `field_schema`:  
    Defines the name and data type of the new field. See [scalar field schema](../create/schema/#scalar-fields) for details.
- `expression`:  
...
```python tab="Python" title="Drop a column"
...
```python tab="Python" title="Alter a column"
# Rename
...
```python tab="Python" title="View the current schema"
...
```python tab="Python" title="Create an index"
import zvec
...
```python tab="Python" title="Drop an index"
