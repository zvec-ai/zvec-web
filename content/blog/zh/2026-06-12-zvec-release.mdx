...
> Zvec v0.5.0 正式发布，核心亮点是原生全文检索（FTS）与全新的 DiskANN 索引。借助 FTS + 向量混合检索，Zvec 将稠密向量、稀疏向量、标量过滤与文本统一到同一个嵌入式引擎。本版本同时统一了查询 API，让多向量查询与重排延迟更低、各语言 SDK 行为一致，加速了 optimize，并提升了索引构建的稳定性。
...
在底层，多目标向量查询与 reranker 逻辑已从 Python 迁移到原生 C++ 引擎。这带来三项具体收益：

+ **更低延迟**：reranker 在引擎内、数据旁就近计算，消除了 Python 边界的序列化与 GIL 竞争。
+ **行为一致**：逻辑只有一份 C++ 实现，多向量查询与重排在所有语言绑定（Python、Go、Rust、C++）上表现完全一致——不会出现各 SDK 在排序或融合上的偏差。
