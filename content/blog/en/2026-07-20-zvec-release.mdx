...
This release adds an optional rotation parameter to INT8/INT4 quantization, now supporting HNSW, Flat, IVF, and Vamana indexes. The benefits are significant — recall improves while maintaining the same QPS:
...
Replaced the previous general-category tokenizer with **Unicode 17 UAX #29 word-boundary tables** generated from official Unicode data. The new tokenizer implements Lucene-style token selection for alphanumeric, numeric, ideographic, hiragana, katakana, hangul, southeast Asian, regional indicator, and common emoji sequences — aligning Zvec's tokenization behavior with Elasticsearch's standard tokenizer.

### UTF-8 Tokenizer & ASCII Folding
Integrated **utf8proc 2.11.3** to upgrade the entire FTS text analysis pipeline with full Unicode support:

+ **StandardTokenizer**: Unicode-aware word boundary detection supporting letters and digits of any script (Latin, Cyrillic, Greek, CJK, etc.)
