...
## Unified Query API & Native Multi-Vector Query
...
Under the hood, the multi-target vector query and reranker logic has been migrated from Python into the native C++ engine. This brings three concrete benefits:
...
+ **Consistent behavior**: A single C++ implementation makes multi-vector query and reranking behave identically across all language bindings (Python, Go, Rust, C++) — no per-SDK drift in ordering or fusion.
